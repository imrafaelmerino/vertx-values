...
public class SendTwoMessageToEventBus {
    ...
    public void vertx_values_obj(Blackhole blackHole) {
        blackHole.consume(vertx.eventBus().request("bounce", immutableJsonObj)
    ...
    public void vertx_values_array(Blackhole blackHole) {
        blackHole.consume(
                vertx.eventBus().request("bounce", immutableJsonArray)
                        .flatMap(message -> vertx.eventBus()
                                                 .request("bounce", message.body()))
                        .toCompletionStage()
                        .toCompletableFuture()
    ...
    public static void main(String[] args) {
        ...
        vertx.deployVerticle(new RegisterJsValuesCodecs()).toCompletionStage().toCompletableFuture().join();
        vertx.eventBus().consumer("bounce", m -> m.reply(m.body()));
        Message<Object> obj =
                vertx.eventBus().request("bounce", immutableJsonArray)
                .flatMap(message -> {
